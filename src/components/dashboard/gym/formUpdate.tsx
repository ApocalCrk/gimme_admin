...
import { useUpdateGym } from './action';
...
export default function FormUpdate({ id }: { id: string }): React.JSX.Element {
    const [gym, setGym] = React.useState<GymType | null>(null);
    const { updateGym, loading } = useUpdateGym();
    ...
    const [packages, setPackages] = React.useState([{ name: '', yearly: '', monthly: '', feature: [{ feature: '' }] }]);

    React.useEffect(() => {
        const handleGetGym = async (): Promise<void> => {
            try {
                const { data, error } = await gymClient.getGym(id);
                if (error) {
                    logger.error('Get gym error', error);
                    return;
                }
                if (data) {
                    setGym(data);
                ...
                logger.error('Get gym error', err);
            }
        };
    ...
    React.useEffect(() => {
        if (gym) {
            ...
            setPlace(gym.place);
            const openCloseTime = gym.open_close_time.split(' - ');
            setOpenTime(openCloseTime[0]);
            setCloseTime(openCloseTime[1]);
            setDescription(gym.description);
            const facilitiesObject = JSON.parse(gym.facilities || '{}');
            const facilitiesArray: { fac: string; icon: string; }[] = Object.values(facilitiesObject.facilities || {});
            setFacilities(facilitiesArray);
            const packagesObject = JSON.parse(gym.packages || '{}');
            const packagesArray = Object.keys(packagesObject.package || {}).map((key) => {
                return {
                    name: key,
                    yearly: packagesObject.package[key].price.yearly as string,
                    monthly: packagesObject.package[key].price.monthly as string,
                    feature: Object.values(packagesObject.package[key].feature || {}).map((feature) => {
                        return { feature: feature as string };
                    }),
                };
            });
